...
# --- Ресурсы (Кэшируем для оптимизации) ---
static var _explosion_effect_script = preload("res://scripts/ExplosionEffect.gd")
static var _smoke_texture = preload("res://assets/future_tanks/PNG/Effects/Smoke_A.png")
...
var _explode_on_wall: bool = false # Флаг второй фазы Boss2
...
func init(is_player_bullet: bool, damage_val: int, splash_dmg_val: int,
		ignored_rid: RID = RID(), bounces: int = DEFAULT_BOUNCES, explosive: bool = false, explode_on_wall: bool = false):
	_is_player = false
	_damage = damage_val
	...
	_explode_on_wall = explode_on_wall
	_update_visuals()
...
func _setup_explosive_visuals():
	...
	if not has_node("Halo"):
		_halo_visual = Node2D.new(); _halo_visual.name = "Halo"; _halo_visual.set_script(_explosion_effect_script); add_child(_halo_visual)
...
func _move(delta):
	...
	if hit:
		...
		if collider is Player:
			...
			if _bounces_left > 0:
				# ВО ВТОРОЙ ФАЗЕ взрываемся при КАЖДОМ соударении
				if _is_explosive and _explode_on_wall:
					_explode_at_bounce()

				# Рикошетим и летим дальше
				...
				# ОТСКОКОВ НЕТ: Финальный взрыв и исчезновение
				_handle_impact(hit.position)
		return
...
func _play_shockwave():
	if not _explosion_effect_script: return
	var effect = Node2D.new(); effect.set_script(_explosion_effect_script); get_parent().add_child(effect); effect.global_position = global_position; effect.z_index = 5
...
func _spawn_explosion_fx():
	...
	var grad = Gradient.new(); grad.set_color(0, Color(1.0, 0.95, 0.3, 1.0)); grad.add_point(0.25, Color(1.0, 0.4, 0.0, 0.95)); grad.add_point(0.25, Color(1.0, 0.4, 0.0, 0.95)); grad.add_point(0.6, Color(0.7, 0.1, 0.0, 0.6)); grad.add_point(1.0, Color(0.15, 0.15, 0.15, 0.0)); expl.color_ramp = grad; get_parent().add_child(expl); get_tree().create_timer(2.0).timeout.connect(expl.queue_free)

	var snd = AudioStreamPlayer2D.new(); snd.stream = _explosion_sound; snd.volume_db = 3.0; snd.pitch_scale = 0.55; snd.bus = "SFX"; get_parent().add_child(snd); snd.global_position = global_position; snd.play(); snd.finished.connect(snd.queue_free)
